...
const separatorsOf = (ctx: {
  generator: { config: { separators?: unknown } };
}): string => {
  const raw = (ctx.generator.config.separators ?? [":", "-"]) as
    | string
    | string[];
  ...
  return {
    ...
    match(matcher, ctx) {
      const m = matcher.match(
        new RegExp(`^${escapeRegExp(name)}(?:${separatorsOf(ctx as never)})`),
      );
      if (!m) return;
...
function responsiveVariant(bp: string, query: string): DaisyVariant {
  return {
    ...
    match(matcher, ctx) {
      const m = matcher.match(
        new RegExp(`^${escapeRegExp(bp)}(?:${separatorsOf(ctx as never)})`),
      );
      if (!m) return;
      const rest = matcher.slice(m[0].length);
      ...
      return {
        ...
        handle: (input, next) =>
          next({
            ...input,
            parent: input.parent ? `${input.parent} $$ ${query}` : query,
          }),
      };
    },
  };
...
export function daisyVariants(opts: {
  prefix: string;
}): Exclude<Preset["variants"], undefined> {
  const prefix = opts.prefix ?? "";
  const out: DaisyVariant[] = [
    drawerVariant(prefix, "open"),
    drawerVariant(prefix, "close"),
  ];
