...
export function batch3Rules(opts: Ctx): Preset["rules"] {
  ...
  if (shouldInclude("link", opts.include, opts.exclude)) {
    ...
    // Expanded: outline-hidden->--tw-outline-style:none + outline-style:none
    // (+ forced-colors fallback, exactly as Tailwind v4 emits it and as the
    // compiled daisyui.css carries it).
    const link = sel(".link");
    rules.push([
      "__daisy-link-state",
      [
        `${link}:focus{--tw-outline-style:none;outline-style:none;@media (forced-colors:active){outline-offset:2px;outline:2px solid #0000;}}` +
  ...
  if (shouldInclude("mask", opts.include, opts.exclude)) {
    ...
    // Computed-identical per CSS mask-position keywords (left=0%, right=100%).
    // `[dir="rtl"]` quoting matches upstream source; compiled drops the
    // quotes (`[dir=rtl]`) — identical matching.
