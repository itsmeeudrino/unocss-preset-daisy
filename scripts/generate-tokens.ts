...
if (variablesJs || variablesCss || lightCss) {
  ...
  if (seen.size > 0) {
    ...
    if (r.length > 0 && covers(r, FALLBACK_RADIUS)) radius = orderByFallback(r, FALLBACK_RADIUS);
    if (s.length > 0 && covers(s, FALLBACK_SIZE)) sizes = orderByFallback(s, FALLBACK_SIZE);
  ...
  console.warn("warn: all theme fetches failed (offline?). Using fallback var set.");
}

const lines: string[] = [];
lines.push(`// Generated by \`bun run tokens\` from ${FORBID_OKLCH()}`);
lines.push(`// Do not hand-edit oklch values. Values live verbatim in src/theme/themes.css.`);
lines.push(
  `// Bridge: upstream CSS vars (packages/daisyui functions/variables.js + variables.css,`,
);
lines.push(`// verified against src/themes/light.css) -> Uno theme.extend.colors.`);
lines.push(``);
lines.push(`/** All daisyUI theme variable names (values live in themes.css). */`);
...
for (const c of colors) {
  const key = /^[A-Za-z_$][A-Za-z0-9_$-]*$/.test(c) && !c.includes("-") ? c : `'${esc(c)}'`;
...
lines.push(`} as const;`);
lines.push(``);
lines.push(`/** \`--radius-*\` bridge. Mirrors upstream variables.js borderRadius. */`);
...
lines.push(` *`);
lines.push(` * Maps Uno \`theme.extend.colors\` to the CSS vars declared by themes.css,`);
lines.push(` * so \`bg-primary\` / \`text-base-content\` etc. resolve to the active theme`);
lines.push(` * (\`:root\` or \`[data-theme=...]\`). Radius/size/depth/noise/border vars`);
lines.push(` * are exposed alongside colors for shortcuts/rules to consume.`);
lines.push(` *`);
lines.push(` * Accepts the resolved preset options (only \`themes\` is read; present for`);
lines.push(` * API symmetry with the other preset parts). Never contains oklch values.`);
lines.push(` */`);
lines.push(
...
lines.push(`      // daisyUI-specific vars (verbatim names; values in themes.css)`);
