...
export function winningDecl(
  ...
  cands.sort(
    (a, b) =>
      (LAYER_RANK[a.layer] ?? 0) - (LAYER_RANK[b.layer] ?? 0) ||
      a.spec - b.spec ||
      a.order - b.order,
  );
  ...
  return w === undefined
    ? null
    : { value: w.value, layer: w.layer, selector: w.selector };
}
...
export function resolveFor(
  ...
  while (true) {
    ...
    if (inline !== null)
      out += resolveFor(rules, classes, theme, inline.value, depth + 1);
    else {
