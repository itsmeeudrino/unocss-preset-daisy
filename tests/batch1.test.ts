...
async function batchUno(opts: Batch1Opts): Promise<Awaited<ReturnType<typeof createGenerator>>> {
  const k = JSON.stringify(opts)
  const hit = genCache.get(k)
  if (hit !== undefined) return hit
  const uno = await createGenerator({
    presets: [presetUno(), { name: 'batch1test', shortcuts: batch1Shortcuts(opts), rules: batch1Rules(opts) ?? [] }],
  })
  genCache.set(k, uno)
  return uno
}

async function genBatch1(classes: string, opts: Batch1Opts = baseOpts): Promise<string> {
...
describe('alert color matrix resolves to theme values', () => {
  ...
  for (const [cls, color] of cases) {
    test(`dark: ${cls} text/bg/border wire to ${color}`, async () => {
      ...
      expect(resolveFor(rules, classes, vars, bd?.value ?? '')).toBe(tvar(vars, `--color-${color}`))
    })
  }
  ...
  test('dark: alert-soft alert-error tints error over base-100', async () => {
    ...
    expect(fg).not.toBeNull()
    expect(bg).not.toBeNull()
    expect(resolveFor(rules, classes, vars, fg?.value ?? '')).toBe(tvar(vars, '--color-error'))
  ...
  test('alert-outline alert-info: info text on transparent bg', async () => {
    ...
    expect(resolveFor(rules, classes, vars, winningDecl(rules, classes, 'color')?.value ?? '')).toBe(
      tvar(vars, '--color-info'),
    )
    expect(resolveFor(rules, classes, vars, winningDecl(rules, classes, 'background-color')?.value ?? '')).toBe('#0000')
  })

  test('alert-dash sets dashed border', async () => {
    const rules = parseCss(await genBatch1('alert alert-dash'))
    expect(winningDecl(rules, ['alert', 'alert-dash'], 'border-style')?.value).toBe('dashed')
  })
...
describe('chat bubble matrix resolves to theme values', () => {
  ...
  test('plain bubble uses base-300', async () => {
    ...
    const vars = await themeVars('dark')
    expect(resolveFor(rules, classes, vars, winningDecl(rules, classes, 'background-color')?.value ?? '')).toBe(
      tvar(vars, '--color-base-300'),
    )
  })

  test('chat-start/end placement', async () => {
    const css = await genBatch1('chat chat-start chat-end chat-bubble chat-header chat-footer chat-image')
...
describe('checkbox matrix resolves to theme values', () => {
  ...
  for (const color of colors) {
    test(`checkbox-${color} sets --input-color`, async () => {
      ...
      expect(fg).not.toBeNull()
      expect(resolveFor(rules, classes, vars, ic?.value ?? '')).toBe(tvar(vars, `--color-${color}`))
      expect(resolveFor(rules, classes, vars, fg?.value ?? '')).toBe(tvar(vars, `--color-${color}-content`))
    })
  }
  ...
  test('sizes match upstream numeric values', async () => {
    ...
    const rules = parseCss(await genBatch1(`checkbox ${Object.keys(expected).join(' ')}`))
  ...
  test('checked/indeterminate/disabled states emit with the base', async () => {
    ...
    expect(css).toContain('clip-path:polygon(20% 100%, 20% 80%, 50% 80%, 50% 0%, 70% 0%, 70% 100%)')
  })
})
...
describe('aura variants and sizes', () => {
  test('sizes match upstream numeric values', async () => {
    ...
    const rules = parseCss(await genBatch1(`aura ${Object.keys(expected).join(' ')}`))
    for (const [cls, pad] of Object.entries(expected)) {
      expect(winningDecl(rules, ['aura', cls], '--aura-padding')?.value).toBe(pad)
    }
  })

  test('base animation + keyframes', async () => {
    const css = await genBatch1('aura')
    expect(css).toContain('animation:aura var(--tw-duration, 6s) linear infinite')
  ...
  test('holo and glow effects', async () => {
    ...
    expect(css).toContain('animation:aura var(--tw-duration, 20s) linear infinite')
    expect(css).toContain('radial-gradient(closest-corner at center, currentColor 0%, transparent 90%)')
  })
})
...
describe('avatar, breadcrumbs, carousel', () => {
  test('avatar-group is flex, avatar is inline-flex', async () => {
    ...
    expect(winningDecl(rules, ['avatar'], 'display')?.value).toBe('inline-flex')
  })

  test('online/offline dots use success/base-300', async () => {
    const css = await genBatch1('avatar-online avatar-offline avatar-placeholder')
  ...
  test('breadcrumbs base + separator', async () => {
    ...
    expect(winningDecl(rules, ['breadcrumbs'], 'margin-inline-start')?.value).toBe('-0.25rem')
  ...
  test('carousel orientations + snap modifiers', async () => {
    const rules = parseCss(await genBatch1('carousel carousel-vertical carousel-item'))
    expect(winningDecl(rules, ['carousel'], 'display')?.value).toBe('inline-flex')
    expect(winningDecl(rules, ['carousel', 'carousel-vertical'], 'flex-direction')?.value).toBe('column')
    expect(winningDecl(rules, ['carousel-item'], 'scroll-snap-align')?.value).toBe('start')
...
describe('calendar roots', () => {
  ...
  test('vc base references theme vars', async () => {
    ...
    expect(winningDecl(rules, ['vc'], 'border')?.value).toContain('var(--color-base-200)')
...
describe('collapse, countdown, diff', () => {
  test('collapse-open forces rows', async () => {
    ...
    expect(winningDecl(rules, ['collapse', 'collapse-open'], 'grid-template-rows')?.value).toBe('max-content 1fr')
  })
  ...
  test('collapse-content is hidden by default', async () => {
    ...
    expect(winningDecl(rules, ['collapse-content'], 'content-visibility')?.value).toBe('hidden')
  })
  ...
  test('diff resizer + items', async () => {
    const rules = parseCss(await genBatch1('diff diff-resizer diff-item-1 diff-item-2'))
    expect(winningDecl(rules, ['diff-resizer'], 'width')?.value).toBe('50cqi')
    expect(winningDecl(rules, ['diff-item-1'], 'border-right')?.value).toContain('var(--color-base-100)')
...
describe('batch1 prefix + include/exclude', () => {
  ...
  test('exclude/include filter per component', async () => {
    const noAlert = await genBatch1('alert alert-error chat', { prefix: '', include: [], exclude: ['alert'] })
    expect(noAlert).not.toContain('.alert')
    expect(noAlert).toContain('.chat')
    const onlyChat = await genBatch1('chat alert', { prefix: '', include: ['chat'], exclude: [] })
