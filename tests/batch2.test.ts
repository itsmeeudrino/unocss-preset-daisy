...
async function cssForBatch2(
  classes: string,
  opts: Batch2Opts = baseOpts,
): Promise<string> {
...
describe("divider", () => {
  ...
  test("base is a stretched flex row with themed line color", async () => {
    ...
    expect(winningDecl(rules, ["divider"], "flex-direction")?.value).toBe(
      "row",
    );
    expect(winningDecl(rules, ["divider"], "margin")?.value).toBe(
      "var(--divider-m, 1rem 0)",
    );
  ...
  test("orientations flip axis and margins", async () => {
    const rules = parseCss(
      await cssForBatch2("divider divider-horizontal divider-vertical"),
    );
    expect(
      winningDecl(rules, ["divider-horizontal"], "--divider-m")?.value,
    ).toBe("0 1rem");
    expect(winningDecl(rules, ["divider-vertical"], "--divider-m")?.value).toBe(
      "1rem 0",
    );
    const css = await cssForBatch2(
  ...
  test("line colors resolve to theme colors", async () => {
    ...
    const rules = parseCss(
      await cssForBatch2(
        "divider " + DIVIDER_COLORS.map((c) => `divider-${c}`).join(" "),
      ),
    );
    for (const color of DIVIDER_COLORS) {
      const rule = rules.find((r) =>
        r.selectors.some((s) => s === `.divider-${color}:before`),
      );
  ...
  test("start/end hide one line", async () => {
    const rules = parseCss(
      await cssForBatch2("divider divider-start divider-end"),
    );
    const start = rules.find((r) =>
      r.selectors.some((s) => s === ".divider-start:before"),
    );
    const end = rules.find((r) =>
      r.selectors.some((s) => s === ".divider-end:after"),
    );
...
describe("dock", () => {
  ...
  test("base is a fixed bottom bar with safe-area height", async () => {
    ...
    expect(winningDecl(rules, classes, "background-color")?.value).toBe(
      "var(--color-base-100)",
    );
    expect(winningDecl(rules, classes, "height")?.value).toContain(
  ...
  test("sizes match upstream heights and label sizes", async () => {
    ...
    for (const [name, h, labelFs] of sizes) {
      expect(winningDecl(rules, [name], "height")?.value).toContain(
        `calc(${h} + env(safe-area-inset-bottom))`,
      );
      const label = rules.find((r) =>
        r.selectors.some((s) => s === `.${name} .dock-label`),
      );
      expect(
        label?.decls.find(([p]) => p === "font-size")?.[1],
        `${name} label`,
      ).toBe(labelFs);
    }
    const xsAfter = rules.find((r) =>
      r.selectors.some((s) => s.includes(".dock-xs") && s.includes(":after")),
    );
    expect(xsAfter?.decls.find(([p]) => p === "bottom")?.[1]).toBe("-0.1rem");
    const lgAfter = rules.find((r) =>
      r.selectors.some((s) => s.includes(".dock-lg") && s.includes(":after")),
    );
  ...
  test("active indicator and base label", async () => {
    ...
    const active = rules.find((r) =>
      r.selectors.some((s) => s === ".dock-active:after"),
    );
    expect(active?.decls.find(([p]) => p === "width")?.[1]).toBe("2.5rem");
    expect(active?.decls.find(([p]) => p === "background-color")?.[1]).toBe(
      "currentColor",
    );
    const label = rules.find((r) =>
      r.selectors.some(
        (s) => s.endsWith(".dock-label") && s.includes("> *:not("),
      ),
    );
    expect(label?.decls.find(([p]) => p === "font-size")?.[1]).toBe(
      "0.6875rem",
    );
  });
});
...
describe("drawer", () => {
  ...
  test("drawer/content/side/toggle base values", async () => {
    const rules = parseCss(
      await cssForBatch2("drawer drawer-content drawer-side drawer-toggle"),
    );
    expect(winningDecl(rules, ["drawer"], "display")?.value).toBe("grid");
    expect(winningDecl(rules, ["drawer"], "grid-auto-columns")?.value).toBe(
      "max-content auto",
    );
    expect(
      winningDecl(rules, ["drawer-content"], "grid-column-start")?.value,
    ).toBe("2");
    expect(winningDecl(rules, ["drawer-side"], "visibility")?.value).toBe(
      "hidden",
    );
    expect(winningDecl(rules, ["drawer-side"], "position")?.value).toBe(
      "fixed",
    );
    expect(winningDecl(rules, ["drawer-side"], "opacity")?.value).toBe("0");
    expect(winningDecl(rules, ["drawer-toggle"], "position")?.value).toBe(
      "fixed",
    );
    expect(winningDecl(rules, ["drawer-toggle"], "appearance")?.value).toBe(
      "none",
    );
    const css = await cssForBatch2(
      "drawer drawer-content drawer-side drawer-toggle",
    );
  ...
  test("overlay, end placement and open state", async () => {
    const rules = parseCss(
      await cssForBatch2(
        "drawer drawer-overlay drawer-end drawer-open drawer-toggle drawer-side drawer-content",
      ),
    );
    const overlay = rules.find((r) =>
      r.selectors.some((s) => s === ".drawer-side > .drawer-overlay"),
    );
    expect(overlay?.decls.find(([p]) => p === "position")?.[1]).toBe("sticky");
    expect(overlay?.decls.find(([p]) => p === "background-color")?.[1]).toBe(
      "oklch(0% 0 0 / 40%)",
    );
    expect(winningDecl(rules, ["drawer-end"], "grid-auto-columns")?.value).toBe(
      "auto max-content",
    );
    const css = await cssForBatch2(
...
describe("dropdown", () => {
  ...
  test("base anchor positioning and content hook", async () => {
    ...
    expect(winningDecl(rules, ["dropdown"], "display")?.value).toBe(
      "inline-block",
    );
    expect(winningDecl(rules, ["dropdown"], "position")?.value).toBe(
      "relative",
    );
    ...
    const content = rules.find(
      (r) =>
        ...
        r.selectors[0] === ".dropdown .dropdown-content",
    );
    expect(content?.decls.find(([p]) => p === "position")?.[1]).toBe(
      "absolute",
    );
  ...
  test("placements set anchors and content geometry", async () => {
    ...
    expect(winningDecl(rules, ["dropdown-start"], "--anchor-h")?.value).toBe(
      "span-inline-end",
    );
    expect(winningDecl(rules, ["dropdown-center"], "--anchor-h")?.value).toBe(
      "center",
    );
    expect(winningDecl(rules, ["dropdown-end"], "--anchor-h")?.value).toBe(
      "span-inline-start",
    );
    expect(winningDecl(rules, ["dropdown-left"], "--anchor-h")?.value).toBe(
      "left",
    );
    expect(winningDecl(rules, ["dropdown-right"], "--anchor-h")?.value).toBe(
      "right",
    );
    expect(winningDecl(rules, ["dropdown-top"], "--anchor-v")?.value).toBe(
      "block-start",
    );
    expect(winningDecl(rules, ["dropdown-bottom"], "--anchor-v")?.value).toBe(
      "block-end",
    );
    const left = rules.find((r) =>
      r.selectors.some((s) => s === ".dropdown-left .dropdown-content"),
    );
    expect(left?.decls.find(([p]) => p === "inset-inline-end")?.[1]).toBe(
      "100%",
    );
    expect(left?.decls.find(([p]) => p === "transform-origin")?.[1]).toBe(
      "100%",
    );
    const top = rules.find((r) =>
      r.selectors.some((s) => s === ".dropdown-top .dropdown-content"),
    );
    expect(top?.decls.find(([p]) => p === "transform-origin")?.[1]).toBe(
      "bottom",
    );
  ...
  test("open shows, close hides", async () => {
    const rules = parseCss(
      await cssForBatch2(
        "dropdown dropdown-open dropdown-close dropdown-hover dropdown-content",
      ),
    );
    ...
    const hidden = rules.find((r) =>
      r.selectors.some(
        (s) => s === ".dropdown.dropdown-close .dropdown-content",
      ),
    );
...
describe("fab", () => {
  ...
  test("base pins to the viewport corner", async () => {
    ...
    expect(winningDecl(rules, ["fab"], "flex-direction")?.value).toBe(
      "column-reverse",
    );
  ...
  test("close/main-action pin and flower fans out", async () => {
    const rules = parseCss(
      await cssForBatch2("fab fab-close fab-main-action fab-flower"),
    );
    for (const name of ["fab-close", "fab-main-action"]) {
      const pinned = rules.find((r) =>
        r.selectors.some((s) => s === `.fab .${name}`),
      );
      expect(pinned?.decls.find(([p]) => p === "position")?.[1], name).toBe(
        "absolute",
      );
      expect(pinned?.decls.find(([p]) => p === "bottom")?.[1], name).toBe("0");
    }
    const flower = rules.find((r) =>
      r.selectors.some((s) => s === ".fab-flower"),
    );
...
describe("fieldset", () => {
  ...
  test("legend and label wire to base-content", async () => {
    const vars = await themeVars("light");
    const rules = parseCss(
      await cssForBatch2("fieldset fieldset-legend fieldset-label"),
    );
    expect(
      winningDecl(rules, ["fieldset"], "grid-template-columns")?.value,
    ).toBe("1fr");
    expect(winningDecl(rules, ["fieldset"], "font-size")?.value).toBe(
      "0.75rem",
    );
    const legend = winningDecl(rules, ["fieldset-legend"], "color");
    expect(
      resolveFor(rules, ["fieldset-legend"], vars, legend?.value ?? ""),
    ).toBe(tvar(vars, "--color-base-content"));
    expect(winningDecl(rules, ["fieldset-legend"], "font-weight")?.value).toBe(
      "600",
    );
    expect(
      winningDecl(rules, ["fieldset-legend"], "margin-bottom")?.value,
    ).toBe("-0.25rem");
    const label = winningDecl(rules, ["fieldset-label"], "color");
    expect(
      resolveFor(rules, ["fieldset-label"], vars, label?.value ?? ""),
    ).toContain(tvar(vars, "--color-base-content"));
...
describe("file-input", () => {
  ...
  test("base sizes off --size like input", async () => {
    ...
    expect(winningDecl(rules, ["file-input"], "height")?.value).toBe(
      "var(--size)",
    );
    expect(winningDecl(rules, ["file-input"], "width")?.value).toBe(
      "clamp(3rem, 20rem, 100%)",
    );
    expect(winningDecl(rules, ["file-input"], "--size")?.value).toBe(
    ...
    for (const color of FILEINPUT_COLORS) {
      ...
      expect(
        resolveFor(
          ...
          cls,
          vars,
          winningDecl(rules, cls, "--input-color")?.value ?? "",
        ),
      ...
      expect(
        resolveFor(
          ...
          cls,
          vars,
          winningDecl(rules, cls, "--btn-color")?.value ?? "",
        ),
      ).toBe(tvar(vars, `--color-${color}`));
      const btn = rules.find((r) =>
        r.selectors.some(
          (s) => s === `.file-input-${color}::file-selector-button`,
        ),
      );
      expect(
        resolveVars(btn?.decls.find(([p]) => p === "color")?.[1] ?? "", vars),
      ).toBe(tvar(vars, `--color-${color}-content`));
    }
  });
  ...
  test("sizes match upstream numeric values", async () => {
    ...
    expect(
      winningDecl(rules, ["file-input-xl"], "padding-inline-end")?.value,
    ).toBe("1.5rem");
  });
  ...
  test("ghost is transparent until focus", async () => {
    ...
    expect(
      winningDecl(rules, ["file-input-ghost"], "background-color")?.value,
    ).toBe("#0000");
    const focus = rules.find((r) =>
      r.selectors.some((s) => s === ".file-input-ghost:focus"),
    );
    expect(focus?.decls.find(([p]) => p === "background-color")?.[1]).toBe(
      "var(--color-base-100)",
    );
  });
});
...
describe("filter", () => {
  ...
  test("children layout and :has collapse", async () => {
    ...
    const reset = rules.find((r) =>
      r.selectors.some((s) => s === ".filter input.filter-reset"),
    );
    expect(reset?.decls.find(([p]) => p === "aspect-ratio")?.[1]).toBe("1");
    const cross = rules.find((r) =>
      r.selectors.some((s) => s === ".filter input.filter-reset::after"),
    );
    expect(cross?.decls.find(([p]) => p === "--tw-content")?.[1]).toContain(
      "×",
    );
    expect(cross?.decls.find(([p]) => p === "content")?.[1]).toBe(
      "var(--tw-content)",
    );
...
describe("footer", () => {
  ...
  test("base grid and title", async () => {
    ...
    expect(winningDecl(rules, ["footer-title"], "text-transform")?.value).toBe(
      "uppercase",
    );
  ...
  test("center/horizontal/vertical flows", async () => {
    const rules = parseCss(
      await cssForBatch2("footer-center footer-horizontal footer-vertical"),
    );
    expect(winningDecl(rules, ["footer-center"], "text-align")?.value).toBe(
      "center",
    );
    expect(winningDecl(rules, ["footer-center"], "grid-auto-flow")?.value).toBe(
      "column dense",
    );
    expect(
      winningDecl(rules, ["footer-horizontal"], "grid-auto-flow")?.value,
    ).toBe("column");
    expect(
      winningDecl(rules, ["footer-vertical"], "grid-auto-flow")?.value,
    ).toBe("row");
    const css = await cssForBatch2(
      "footer-center footer-horizontal footer-vertical",
    );
...
describe("hero", () => {
  ...
  test("overlay dims with neutral, content caps width", async () => {
    const vars = await themeVars("light");
    const rules = parseCss(
      await cssForBatch2("hero hero-overlay hero-content"),
    );
    expect(winningDecl(rules, ["hero"], "display")?.value).toBe("grid");
    expect(winningDecl(rules, ["hero"], "background-size")?.value).toBe(
      "cover",
    );
    const overlay = winningDecl(rules, ["hero-overlay"], "background-color");
    expect(overlay?.value).toContain("var(--color-neutral)");
    expect(
      resolveFor(rules, ["hero-overlay"], vars, overlay?.value ?? ""),
    ).toContain(tvar(vars, "--color-neutral"));
    expect(winningDecl(rules, ["hero-content"], "max-width")?.value).toBe(
      "80rem",
    );
    expect(winningDecl(rules, ["hero-content"], "isolation")?.value).toBe(
      "isolate",
    );
...
describe("hover-3d", () => {
  ...
  test("perspective stage with tilt zones", async () => {
    ...
    expect(winningDecl(rules, ["hover-3d"], "display")?.value).toBe(
      "inline-grid",
    );
    expect(winningDecl(rules, ["hover-3d"], "perspective")?.value).toBe(
      "75rem",
    );
