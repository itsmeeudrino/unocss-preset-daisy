  ...
  test("input-primary sets --input-color", async () => {
    ...
    expect(
      resolveFor(
        ...
        classes,
        vars,
        winningDecl(rules, classes, "--input-color")?.value ?? "",
      ),
...
describe("card, modal, menu, join, glass", () => {
  test("card-border references base-200", async () => {
    ...
    expect(
      winningDecl(rules, ["card", "card-border"], "border")?.value,
    ).toContain("var(--color-base-200)");
  });
  ...
  test("modal-box uses base-100", async () => {
    ...
    expect(
      resolveFor(
        ...
        ["modal-box"],
        vars,
        winningDecl(rules, ["modal-box"], "background-color")?.value ?? "",
      ),
  ...
  test("join-item radii follow join vars", async () => {
    ...
    expect(
      winningDecl(rules, ["join-item"], "border-start-start-radius")?.value,
    ).toBe("var(--join-ss)");
    expect(winningDecl(rules, ["join-item"], "border-width")?.value).toBe(
      "var(--border, 1px)",
    );
  });
  ...
  test("glass is frosted", async () => {
    ...
    expect(winningDecl(rules, ["glass"], "backdrop-filter")?.value).toContain(
      "blur(",
    );
    expect(winningDecl(rules, ["glass"], "background-color")?.value).toBe(
      "#0000",
    );
  });
});
...
describe("theme color utilities", () => {
  test("bg-/text-/border- map to theme vars with opacity via color-mix", async () => {
    const rules = parseCss(
      await cssFor("bg-primary text-accent border-error bg-success/50"),
    );
    ...
    expect(
      winningDecl(rules, ["bg-success/50"], "background-color")?.value,
    ).toBe("color-mix(in oklab, var(--color-success) 50%, #0000)");
  });
});
