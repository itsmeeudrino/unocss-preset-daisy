...
const CORE_TOKENS = [
  'btn', 'btn-primary', 'badge', 'card', 'card-body', 'input',
  'modal-box', 'menu', 'join', 'glass', 'bg-primary', 'rounded-box',
] as const

// Tokens proven eaten under default dash-form variants (see header).
const EATEN_UNDER_DEFAULT = ['hover-3d', 'link-primary', 'link-hover', 'file-input-primary']
...
function exactClassRe(token: string): RegExp {
  return new RegExp(`\\.${token.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}(?![\\w-])`)
}

async function gen(separators: string[]): Promise<Gen> {
  return createGenerator({ presets: [presetUno(), presetDaisy()], separators } as never)
}
...
describe('separators requirement (dash-variant sweep)', () => {
  ...
  test('under separators [":"] every token is styled (own class or via parent)', async () => {
    ...
    for (const t of tokens) {
      ...
      if (!pcss || !exactClassRe(t).test(pcss)) broken.push(`${t} (not styled by parent ${parent})`)
    }
