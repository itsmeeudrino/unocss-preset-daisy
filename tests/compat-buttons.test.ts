...
describe("button color matrix resolves to theme values", () => {
  ...
  test("dark: accent/info/success/warning/error text is dark (reference rendering)", async () => {
    ...
    const css = await cssFor(
      "btn btn-accent btn-info btn-success btn-warning btn-error",
    );
    const rules = parseCss(css);
    for (const color of [
      "accent",
      ...
      const classes = ["btn", `btn-${color}`];
      const fg = winningDecl(rules, classes, "color");
      const l = oklchLightness(
        resolveFor(rules, classes, vars, fg?.value ?? ""),
      );
  ...
  test("plain btn uses base colors", async () => {
    ...
    expect(
      resolveFor(
        ...
        ["btn"],
        vars,
        winningDecl(rules, ["btn"], "color")?.value ?? "",
      ),
    ...
    expect(
      resolveFor(
        ...
        ["btn"],
        vars,
        winningDecl(rules, ["btn"], "background-color")?.value ?? "",
      ),
...
describe("button style variants", () => {
  ...
  test("ghost: transparent bg and border", async () => {
    ...
    expect(
      resolveFor(
        ...
        classes,
        vars,
        winningDecl(rules, classes, "--btn-bg")?.value ?? "",
      ),
    ).toBe("#0000");
  });
  ...
  test("link: underline with primary color", async () => {
    ...
    const rules = parseCss(await cssFor(classes.join(" ")));
    expect(winningDecl(rules, classes, "text-decoration-line")?.value).toBe(
      "underline",
    );
    expect(
      resolveFor(
  ...
  test("sizes match upstream values", async () => {
    const rules = parseCss(
      await cssFor("btn btn-xs btn-sm btn-md btn-lg btn-xl"),
    );
...
describe("button states keep modifier colors", () => {
  test("hover inherits --btn-fg (never hardcodes base-content)", async () => {
    ...
    const hover = rules.find(
      (r) => r.media !== null && r.selectors.some((s) => s.includes(":hover")),
    );
    expect(hover).toBeDefined();
    expect(hover?.decls.find(([p]) => p === "color")?.[1]).toBe(
      "var(--btn-fg)",
    );
  });
  ...
  test("disabled (bare layer) beats base", async () => {
    ...
    expect(
      w?.value.startsWith("color-mix(in oklch, var(--color-base-content)"),
    ).toBe(true);
  });
});
