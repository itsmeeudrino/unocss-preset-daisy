...
describe("applyPrefix", () => {
  ...
  test("prefixes classes incl. pseudo/combinator contexts", () => {
    expect(applyPrefix(".btn:active:not(.btn-active)", "d-")).toBe(
      ".d-btn:active:not(.d-btn-active)",
    );
    expect(applyPrefix(".join > :where(.btn)", "d-")).toBe(
      ".d-join > :where(.d-btn)",
    );
  ...
  test("never corrupts decimals, urls or attribute values", () => {
    expect(
      applyPrefix(
        ".btn{transition-timing-function:cubic-bezier(0, 0, 0.2, 1)}",
        "d-",
      ),
    ).toBe(".d-btn{transition-timing-function:cubic-bezier(0, 0, 0.2, 1)}");
    expect(applyPrefix(".a{width:0.5rem;gap:.25rem}", "d-")).toBe(
      ".d-a{width:0.5rem;gap:.25rem}",
    );
    expect(applyPrefix('.a{background:url("icon.svg")}', "d-")).toBe(
      '.d-a{background:url("icon.svg")}',
    );
    expect(applyPrefix('[aria-current="false"]', "d-")).toBe(
      '[aria-current="false"]',
    );
  });
});
