...
async function gen(
  ...
  const uno = await createGenerator({
    presets: [
      ...
      {
        name: "batch5test",
        shortcuts: batch5Shortcuts(ctx),
        rules: batch5Rules(ctx) ?? [],
      },
    ],
  });
  genCache.set(k, uno);
  ...
  test("step colors wire --step-bg/--step-fg to theme values", async () => {
    ...
    for (const theme of ["light", "dark"] as const) {
      ...
      for (const [cls, color] of colors) {
        ...
        expect(resolveVars(decl(rule, "--step-bg"), vars)).toBe(
          tvar(vars, `--color-${color}`),
        );
        expect(resolveVars(decl(rule, "--step-fg"), vars)).toBe(
...
describe("tab", () => {
  test("container + tab frame vars", async () => {
    ...
    expect(winningDecl(rules, ["tab"], "height")?.value).toBe(
      "var(--tab-height)",
    );
  ...
  test("tab-content is hidden order-1 box", async () => {
    ...
    expect(
      winningDecl(rules, ["tab-content"], "--tabcontent-order")?.value,
    ).toBe("1");
  });

  test("tab-disabled is inert", async () => {
    const rules = await rulesFor("tab-disabled");
    expect(winningDecl(rules, ["tab-disabled"], "pointer-events")?.value).toBe(
      "none",
    );
  ...
  test("tabs-box active maps to base-100", async () => {
    ...
    expect(resolveVars(decl(rule, "background-color"), vars)).toBe(
...
describe("table", () => {
  test("base is separated with box radius", async () => {
    ...
    expect(winningDecl(rules, ["table"], "border-collapse")?.value).toBe(
      "separate",
    );
    expect(winningDecl(rules, ["table"], "border-radius")?.value).toBe(
      "var(--radius-box)",
    );
  ...
  test("sizes match upstream numeric values", async () => {
    ...
    for (const [cls, [fs, pb, pi]] of Object.entries(expected)) {
      expect(
        findRule(
          rules,
          `.${cls} :not(thead, tfoot) tr`,
          "font-size",
        )?.decls.find(([p]) => p === "font-size")?.[1],
      ).toBe(fs);
...
describe("textarea", () => {
  test("base geometry", async () => {
    ...
    expect(winningDecl(rules, ["textarea"], "width")?.value).toBe(
      "clamp(3rem, 20rem, 100%)",
    );
    expect(winningDecl(rules, ["textarea"], "border-radius")?.value).toBe(
      "var(--radius-field)",
    );
  });
  ...
  test("ghost is transparent until focus", async () => {
    ...
    expect(
      winningDecl(rules, ["textarea", "textarea-ghost"], "background-color")
        ?.value,
    ).toBe("#0000");
    ...
    expect(css).toContain(
      "&:focus,&:focus-within{background-color:var(--color-base-100);color:var(--color-base-content);",
    );
    expect(resolveVars("var(--color-base-content)", vars)).toBe(
  ...
  test("sizes match upstream numeric values", async () => {
    const rules = await rulesFor(
      "textarea textarea-xs textarea-sm textarea-md textarea-lg textarea-xl",
    );
    ...
    for (const [cls, [fs, top]] of Object.entries(expected)) {
      expect(
        winningDecl(rules, ["textarea", cls], "--font-size-min")?.value,
      ).toBe(fs);
...
describe("textrotate", () => {
  test("base is a clipped inline block, exactly as upstream does", async () => {
    ...
    expect(winningDecl(rules, ["text-rotate"], "overflow")?.value).toBe(
      "hidden",
    );
...
describe("timeline", () => {
  test("base + boxes + slots", async () => {
    ...
    expect(resolveFor(rules, ["timeline-box"], vars, box?.value ?? "")).toBe(
      tvar(vars, "--color-base-100"),
    );
    expect(winningDecl(rules, ["timeline-start"], "grid-area")?.value).toBe(
      "1/1/2/4",
    );
    expect(
      winningDecl(rules, ["timeline-middle"], "grid-column-start")?.value,
    ).toBe("2");
    expect(winningDecl(rules, ["timeline-end"], "grid-area")?.value).toBe(
      "3/1/4/4",
    );
  });
  ...
  test("orientations + compact + snap keep upstream selectors", async () => {
    const css = await cssFor5(
      "timeline-vertical timeline-horizontal timeline-compact timeline-snap-icon",
    );
    for (const needle of [
...
describe("toggle", () => {
  test("base geometry", async () => {
    ...
    expect(winningDecl(rules, ["toggle"], "grid-template-columns")?.value).toBe(
      "0fr 1fr 1fr",
    );
  ...
  for (const color of [
    ...
    test(`toggle-${color} wires checked --input-color to theme`, async () => {
      for (const theme of ["light", "dark"] as const) {
        ...
        expect(resolveVars(decl(rule, "--input-color"), vars)).toBe(
          tvar(vars, `--color-${color}`),
        );
      }
    });
...
describe("tooltip", () => {
  test("base vars point at neutral", async () => {
    ...
    expect(resolveFor(rules, ["tooltip"], vars, bg?.value ?? "")).toBe(
  ...
  test("positions keep upstream transforms", async () => {
    ...
    expect(winningDecl(rules, ["tooltip-start"], "--tt-trans")?.value).toBe(
      "0",
    );
    expect(winningDecl(rules, ["tooltip-center"], "--tt-trans")?.value).toBe(
      "-50%",
    );
    expect(winningDecl(rules, ["tooltip-end"], "--tt-tail-inset")?.value).toBe(
      "auto var(--tt-tail-off)",
    );
    const css = await cssFor5(
  ...
  for (const color of [
    ...
    test(`tooltip-${color} wires bubble + text to theme`, async () => {
      for (const theme of ["light", "dark"] as const) {
        ...
        const text = findRule(
          rules,
          `.tooltip-${color}>.tooltip-content`,
          "color",
        );
        expect(resolveVars(decl(text, "color"), vars)).toBe(
          tvar(vars, `--color-${color}-content`),
        );
      }
    });
...
describe("validator", () => {
  test("hint starts hidden", async () => {
    ...
    expect(winningDecl(rules, ["validator-hint"], "font-size")?.value).toBe(
      "0.75rem",
    );
  });
  ...
  test(":user-valid/:user-invalid wire --input-color, verbatim selectors", async () => {
    ...
    expect(resolveVars(decl(ok, "--input-color"), vars)).toBe(
      tvar(vars, "--color-success"),
    );
    const bad = findRule(
      rules,
    ...
    expect(resolveVars(decl(bad, "--input-color"), vars)).toBe(
  ...
  test("invalid reveals the hint, unhide wins from utilities layer", async () => {
    ...
    expect(resolveVars(decl(hint, "color"), vars)).toBe(
      tvar(vars, "--color-error"),
    );
...
describe("batch5 tokens fixture", () => {
  ...
  test("tests/fixtures/batch5-tokens.json matches the demo and generates CSS", async () => {
    ...
    const html = await Bun.file(
      new URL("../example/sections/batch5.html", import.meta.url),
    ).text();
    const used = new Set<string>();
    for (const m of html.matchAll(/class="([^"]+)"/g)) {
      const cls = m[1];
      if (cls !== undefined)
        for (const t of cls.split(/\s+/)) if (t) used.add(t);
    }
    const demoed = [...used].filter((t) =>
