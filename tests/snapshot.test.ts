...
describe("daisy snapshot", () => {
  ...
  test("prefix option renames utilities and variants", async () => {
    const css = await cssFor(
      '<button class="d-btn d-bg-primary d-is-drawer-open:d-text-accent"></button>',
      {
        prefix: "d-",
      },
    );
  ...
  test("theme tokens present", () => {
    for (const opts of [{}, { themes: ["light", "dark"] }] as DaisyOptions[]) {
      ...
      const colors = (
        preset.theme as unknown as { colors?: Record<string, string> }
      )?.colors;
  ...
  test("join utilities work", async () => {
    const css = await cssFor(
      '<div class="join join-horizontal"><button class="join-item"></button></div>',
    );
    expect(css).toContain("display:inline-flex");
  ...
  test("drawer variants stay top-level", async () => {
    const css = await cssFor(
      '<div class="is-drawer-open:bg-primary is-drawer-close:text-accent"></div>',
    );
  ...
  test("exclude option filters components", async () => {
    const css = await cssFor(
      '<button class="btn"></button><span class="badge"></span>',
      {
        exclude: ["button"],
      },
    );
